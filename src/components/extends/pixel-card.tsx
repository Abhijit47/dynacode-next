...
export default function PixelCard({
  ...
  const doAnimate = (fnName: 'appear' | 'disappear') => {
    const performFrame = () => {
      ...
      if (timePassed < timeInterval) {
        animationRef.current = requestAnimationFrame(performFrame);
        return;
      }
  ...
  return (
    <div
      ref={containerRef}
      className={`relative overflow-hidden grid place-items-center aspect-4/5 border border-[#27272a] rounded-[25px] isolate transition-colors duration-200 ease-[cubic-bezier(0.5,1,0.89,1)] select-none ${className}`}
